...
# Optional: Load existing conversation on startup
# Comment out these lines if you don't want to load previous conversations
...
# # Show alert if user clicked "Chat with us"
# if st.session_state.focus_chat:
#     st.sidebar.success("👋 Hi! How can I help you today?")
#     st.session_state.focus_chat = False
...
#     payload = {"user_query": prompt, "session_id": "streamlit_session"}
#     try:
#         response = requests.post(f"{API_URL}/ask", json=payload)
#         if response.status_code == 200:
#             data = response.json()
#             ai_message = data.get("bot_response", "No response from model")
#         else:
#             ai_message = f"⚠️ Error {response.status_code}: {response.text}"
#     except requests.exceptions.RequestException as e:
#         ai_message = f"⚠️ Connection error: {e}"
...
# # --- Main Page Content ---
# page_content = {
#     "Who We Are": "We are a global leader in consulting, strategy, and digital transformation.",
#     "What We Do": "We solve business challenges using AI, cloud, and cutting-edge tech.",
#     "Our Thinking": "Innovation drives us. We publish insights on market trends regularly.",
#     "Careers": "Join us and be part of a team shaping the future of technology."
# }

# # Create tabs
# tabs = st.tabs(list(page_content.keys()))

# for tab, (tab_name, content) in zip(tabs, page_content.items()):
#     with tab:
#         st.header(tab_name)
#         st.write(content)
        
#         # Create unique key for each tab's nudge state
#         nudge_key = f"nudge_{tab_name}"
        
#         # Check if nudge has been shown for THIS specific tab
#         if nudge_key not in st.session_state.nudge_shown:
#             st.session_state.nudge_shown[nudge_key] = False
        
#         # Show nudge if not already shown for this tab
#         if not st.session_state.nudge_shown[nudge_key]:
#             st.info("👉 Looks like you're interested — want a personalized demo?")
            
#             col1, col2, col3 = st.columns([1, 2, 5])
#             with col1:
#                 if st.button("💬 Chat with us", key=f"chat_btn_{tab_name}"):
#                     st.session_state.focus_chat = True
#                     st.rerun()
#             with col2:
#                 if st.button("Dismiss", key=f"dismiss_btn_{tab_name}"):
#                     st.session_state.nudge_shown[nudge_key] = True
#                     st.rerun()
